...
export const useTriviaGame = create<TriviaGameState>()(
  subscribeWithSelector((set, get) => ({
    ...
    startGame: () => {
      const { settings } = get();
      const questions = getRandomQuestions(settings.numberOfQuestions, settings.categories);
    ...
    showQuestionAnswer: () => {
      const { questions, currentQuestionIndex, selectedAnswers, players } = get();
    ...
    nextQuestion: () => {
      const { questions, currentQuestionIndex, settings, timeRemaining } = get();
